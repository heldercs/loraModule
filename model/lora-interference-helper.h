...
public:
  ...
  public:
    Event (Time duration, double rxPowerdBm, uint8_t spreadingFactor, Ptr<Packet> packet,
           double frequencyMHz, uint16_t nodeId, uint8_t irType);
    ~Event ();
   ...
   */
  void  ClearIndexUmap(uint16_t idx);
...
private:
  ...
  std::unordered_map<uint16_t, std::vector<std::vector<double>>> m_chaseCombiningSnir;
